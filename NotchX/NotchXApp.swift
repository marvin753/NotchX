...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("NotchX", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var viewModels: [String: NotchXViewModel] = [:] // UUID -> NotchXViewModel
    var window: NSWindow?
    let vm: NotchXViewModel = .init()
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? NotchXSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? NotchXSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createNotchXWindow(for screen: NSScreen, with viewModel: NotchXViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = NotchXSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func playWelcomeSound() {
        ...
        audioPlayer.play(fileName: "notchx", fileExtension: "m4a")
    }
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = NotchXViewModel(screenUUID: uuid)
                    let window = createNotchXWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createNotchXWindow(for: selectedScreen, with: vm)
            }
