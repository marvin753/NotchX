...
struct DownloadArea: View {
    ...
    var body: some View {
        if let firstDownload = watcher.downloadFiles.first {
            HStack(alignment: .center) {
                HStack {
                    if firstDownload.browser == .safari {
                ...
                HStack(spacing: 12) {
                    VStack(alignment: .trailing) {
                        Text(firstDownload.formattedSize)
                        Text(firstDownload.name).font(.caption2).foregroundStyle(.gray)
                    }
                }
            }
        } else {
